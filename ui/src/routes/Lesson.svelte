<script>
    ...
    import { getStoredLessons, isLoading, isSidebarVisible, showAlert, storeLessons } from "../lib/store";
    ...
    async function completeCourse(lessonId) {
        ...
        if (progressRecord.status === "In Progress") {
            ...
            if (updatedProgressRecord) {
                ...
                showAlert(
                        `${ currentCourse.title.length > 30
                            ? currentCourse.title.slice(0,
                                                        30) + "..."
                            : currentCourse.title } completed successfully`,
                        "success",
                );
            }
        }
...
{#if $currentUser}
    <main class="flex h-dvh justify-between lg:overflow-x-hidden">
        ...
        {:else if $lessons.length === 0 || $lessons.every(
                (lesson) => slugify(lesson.title, {lower: true, strict: true},) !== $lessonLocation.pathname.slice(1))}
            <NotFound page="lesson" />
        {:else}
            {#each $lessons as lesson (lesson.id)}
                {#if slugify(lesson.title, {lower: true, strict: true},) === slugify(lessonTitle,
              ...
              : "flex flex-1 flex-col justify-between overflow-y-scroll bg-dark p-5"}
                            ...
                            >
                                ...
                                {#if currentCourseStatus === "In Progress" || currentCourseStatus === "Completed"}
                                    <div class="flex items-center gap-3 sm:w-full sm:flex-col">
                                        {#if !findCurrentLessonIndex(getCourseLessons($lessons.find(
                                                (lesson) => slugify(lesson.title,
                                                                    {lower: true, strict: true},) === slugify(
                                                        lessonTitle, {lower: true, strict: true},)).course)) <= 0}
                                            <button
                                        ...
                                        {#if findCurrentLessonIndex(getCourseLessons($lessons.find(
                                                (lesson) => slugify(lesson.title,
                                                                    {lower: true, strict: true},) === slugify(
                                                        lessonTitle,
                                                        {lower: true, strict: true},)).course)) >= getCourseLessons(
                                                $lessons.find((lesson) => slugify(lesson.title, {
                                                    lower: true,
                                                    strict: true
                                                },) === slugify(lessonTitle,
                                                                {lower: true, strict: true},)).course).length - 1}
                                            <button
                        ...
                        currentCourseStatus === "Completed"
                          ...
                          : "line-clamp-1 flex items-center justify-center gap-2 truncate rounded-md bg-emerald-400/60 px-4 py-2 transition hover:bg-emerald-400/50 sm:order-first sm:w-full"}
                            ...
                            {#if lesson.videoRemoteUrl}
                                ...
                                {:else if isGoogleDriveUrl(lesson.videoRemoteUrl) && getGoogleDriveFileId(
                                        lesson.videoRemoteUrl)}
                                    <div class="relative aspect-video w-full overflow-hidden rounded-md">
                                        <iframe
                                ...
                                {:else}
                                    <div class="flex items-center justify-center rounded-md bg-white/5 p-8">
                                        <div class="text-center">
                                            ...
                                            <p class="text-sm text-white/30">Only YouTube and Google Drive URLs are
                                                                             supported</p>
                                        </div>
                                    </div>
                                {/if}
                    ...
                    prose-ul:list-inside prose-ul:list-disc prose-ul:space-y-2 prose-ul:text-base prose-img:w-full prose-img:rounded-md"
                        ...
                        >
                            ...
                            {#if $lesson_resources.filter(
                                    (resource) => resource.lesson.includes(lesson.id),).length > 0}
                                <div class="flex-1 space-y-4 md:w-full">
                                    <h2 class="flex items-center gap-2 text-base">
