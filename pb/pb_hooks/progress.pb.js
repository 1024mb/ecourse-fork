...
onRecordUpdateRequest((e) => {
    ...
    const newAssignees = updatedAssignees.filter((assignee) => !originalAssignees.includes(assignee));
    const removedAssignees = originalAssignees.filter((assignee) => !updatedAssignees.includes(assignee));
    const notStartedId = utils.getProgressTypeId({ progressTypeName: "not_started" });
    ...
    removedAssignees.forEach((assignee) => {
        const progressRecords = $app
            .findAllRecords(progressCollection.name, $dbx.hashExp({ assignee: `${assignee}`, course: `${updatedRecord.id}` }));
    ...
    if (courseId) {
        $app.runInTransaction((txApp) => {
            ...
            if (courseRecord) {
                ...
                const updatedAssignees = assignees.filter((assignee) => assignee !== assigneeToRemove);
...
onRecordCreateRequest((e) => {
    ...
    const assignedToEveryoneCourses = $app.findAllRecords(coursesCollection.name, $dbx.hashExp({ assign_to_everyone: true }));
    const notStartedId = utils.getProgressTypeId({ progressTypeName: "not_started" });
