...
import type { Lesson } from "@/types/collections";
...
const filteredLessonsClass = computed<string>(() => {
    if (filteredLessons.value.length > 5) {
        return "hide-scrollbar relative m-auto h-[461px] w-[700px] overflow-y-scroll rounded-md bg-dark outline outline-[1.5px] outline-white/10";
    } else {
        return "hide-scrollbar relative m-auto w-[700px] overflow-y-scroll rounded-md bg-dark outline outline-[1.5px] outline-white/10";
    }
});
...
<template>
    <Transition name="scale-fade">
        <div
            ...
            :class="filteredLessonsClass"
            aria-hidden="true"
            @click.stop
        >
            <div
                class="
                    sticky inset-x-0 top-0 flex w-full items-center justify-between gap-2 border-b-[1.5px]
                    border-b-white/10 bg-dark px-4
                "
            >
                <Icon
                ...
                >
                    ...
                    >
                        <div
                            ...
                            class="
                                flex w-full cursor-pointer items-center justify-between gap-5 rounded-md bg-transparent
                                p-3 transition
                                hover:bg-white/5
                            "
